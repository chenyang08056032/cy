...
python3 -m sglang.launch_server \
        ...
        --disable-radix-cache \
        --trust-remote-code \
        --host ${D_IP[$i]} --max-running-requests 128 \
        --mem-fraction-static 0.85 \
        --port 8000 \
        --cuda-graph-bs 2 4 6 8 10 16 20 24 28 32 48 64 \
        --enable-multimodal \
        --mm-attention-backend ascend_attn --max-total-tokens 120000 \
        --dtype bfloat16 --mamba-ssm-dtype bfloat16 --disaggregation-mode decode --disaggregation-transfer-backend ascend --base-gpu-id 12